    ...
    return [
        mcp_types.Tool(
            ...
            description=(
                ...
                "result is below the confidence threshold. Each result is a lean subset "
                "of fields (filename, doc_id, page, page_label, chunk, text, project, "
                "score, confidence, retrieval, breadcrumb, links_to, cross_references, "
                "notable) — use engra_list_files/engra_info for file-level metadata."
            ),
...
def _lean_result(hit: dict) -> dict:
    """Trim a full-detail search result down to what an agent needs to read/cite/navigate.

    Drops source (redundant once doc_id is present), total_pages (available via
    engra_info/engra_list_files), indexed_at/source_mtime (staleness belongs to
    engra_list_files, not a per-chunk hit), and linked_from (always empty today).
    """
    return {field: hit[field] for field in _LEAN_RESULT_FIELDS if field in hit}
